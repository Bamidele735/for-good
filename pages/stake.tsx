...
const Stake: NextPage = () => {
  ...
  const {
    ...
    isLoading,
    error,
  } = useContract(stakingContractAddress, stakingAbi);
  ...
  console.log("claimableRewards", error);

  useEffect(() => {
    if (!stakingContract || !address) return;

    async function loadClaimableRewards() {
      const stakeInfo = await stakingContract?.call("availableRewards", [
        address,
      ]);
  ...
  if (isLoading) {
    return (
      <div className="center ">
        Loading <span className="ml-3"> {/* <Spinner /> */}</span>{" "}
      </div>
    );
  }
